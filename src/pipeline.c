...
void createChilds(int * pipe1, int * pipe2) {
    ...
    if (child1 == 0) {
        ...
        int child2 = fork();
        if (child2 == 0) { groupFrequencies(pipe2); }
    }
}
...
void printLetters(int * frequencies) {
    ...
    {
        if (frequencies[i] > 0) { printf("La lettre %c apparaît %d fois.\n", i + 'a', frequencies[i]); }
    }
    for (int i = 0; i < 26; i++) // Upper case
    {
        if (frequencies[i + 26] > 0) { printf("La lettre %c apparaît %d fois.\n", i + 'A', frequencies[i + 26]); }
    }
}
...
void summateDigits(int * pipe) {
    ...
    int sum = 0;
    for (unsigned long i = 0; i < (strlen(digit)); i++) { sum = sum + (digit[i] - 48); }
    if (sum > 0) { fprintf(stdout, "La somme des chiffres est %d.\n", sum); }
    exit(EXIT_SUCCESS);
}
